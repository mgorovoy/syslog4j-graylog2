...
public abstract class AbstractSyslogServer implements SyslogServerIF {
    ...
    protected static boolean isStructuredMessage(SyslogCharSetIF syslogCharSet, String receiveData) {
        // If there's a numerical VERSION field after the <priority> followed by SPACE character, return true
        return RFC5424_REGEX.matcher(receiveData).matches();
    }

    protected static boolean isCiscoMessage(SyslogCharSetIF syslogCharSet, String receiveData) {
        // If there's a numerical SERIAL field after the <priority> followed by COLON character, return true
        return CISCO_REGEX.matcher(receiveData).matches();
    }

    protected static boolean isFortiGateMessage(SyslogCharSetIF syslogCharSet, String receiveData) {
        // If there's a 'date=' after the <priority> , return true
        return FORTI_REGEX.matcher(receiveData).matches();
    }
